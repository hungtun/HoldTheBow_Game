...
namespace Hobow_Server.Physics
{
    public class ServerPhysicsManager
    {
        ...
        private readonly Dictionary<int, EnemyPhysicsBody> _enemyBodies = new();
        ...
        public bool CreateEnemyBody(int enemyId, Vector2 position, float radius = 0.25f)
        {
            if (_enemyBodies.ContainsKey(enemyId))
            {
                return false;
            }
            
            _enemyBodies[enemyId] = new EnemyPhysicsBody
            {
                Position = position,
                Radius = radius,
                Velocity = Vector2.Zero
        ...
        public void RemoveEnemyBody(int enemyId)
        {
            if (_enemyBodies.Remove(enemyId))
            {
            }
        }
        
        public void SetEnemyPosition(int enemyId, Vector2 position)
        {
            if (_enemyBodies.TryGetValue(enemyId, out var body))
            {
        ...
        private bool CheckCollisionWithMap(Vector2 position, float radius)
        {
        ...
        public MapData GetMapData(string mapId)
        {
            try
            {
                if (!_mapCollisions.ContainsKey(mapId))
                {
                    ...
                    return new MapData { MapId = mapId, Name = mapId, Width = 25, Height = 25, TileWidth = 16, TileHeight = 16 };
                }
